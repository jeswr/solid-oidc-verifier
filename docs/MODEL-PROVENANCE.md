...
| `ReplayStore::contains` read-only probe (`src/replay.rs` trait method + `InMemoryReplayStore` impl + tests; `AlwaysErr`/`ErrReplay` stub updates in `src/replay.rs`/`tests/verifier.rs`) | Claude Opus 4.8 | 2026-06-24 | Branch `feat/replay-contains-probe`. Adds a strictly READ-ONLY existence probe to the `ReplayStore` trait — the prerequisite for the consumer (solid-server-rs) jti replay PRE-CHECK optimization (probe before the expensive ES256 proof verify, authoritative `mark` still required after full validation). INV-4 documented + enforced by design: `contains` never mutates; the atomic check-and-set in `mark` stays the single source of truth. Security-critical — re-review candidate. |
